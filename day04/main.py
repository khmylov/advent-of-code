...
def is_valid2(item):
    if not is_valid(item): return False
    ...
    if re.compile(r'^#[0-9a-f]{6}$').match(item['hcl']) is None: return False
    if re.compile(r'^(amb|blu|brn|gry|grn|hzl|oth)$').match(item['ecl']) is None: return False
    if re.compile(r'^[0-9]{9}$').match(item['pid']) is None: return False
    return True
