...
let run (grid, initialCarts) =
  let rec loop step carts =
    // Keep separate lists of carts already moved at this step and remaining ones.
    let rec cartLoop completedCarts remainingCarts =
      ...
      | None -> 
        ...
        | cart::nextRemaining -> cartLoop ((Grid.getNext grid cart)::completedCarts) nextRemaining
      | crash -> crash
    cartLoop [] carts
  loop 0 initialCarts
