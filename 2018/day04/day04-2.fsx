...
let getMinutes (start: DateTimeOffset) (finish: DateTimeOffset) =
  start |> List.unfold (fun date -> if date >= finish then None else Some (date.Minute, date.AddMinutes 1.0))
...
|> Seq.map (fun (guardId, minutes) ->
  let maxGroup = minutes |> Seq.groupBy id |> Seq.maxBy (snd >> Seq.length)
  guardId, (fst maxGroup), (maxGroup |> snd |> Seq.length))
|> Seq.maxBy (fun (_, _, minuteCount) -> minuteCount)
|> fun (guardId, minute, _) -> guardId * minute
